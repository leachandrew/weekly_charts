...
cer_gas_prod<-readxl::read_excel("gas_prod.xlsx", sheet = "2000+ - 103m3d 103m3j", range = NULL, col_names = TRUE,
  ...
  group_by(Region)%>%
  fill(Production, .direction=c("downup")) %>%
  filter(!is.na(Date))

ggplot(cer_gas_prod%>%filter(WCSB)) +
  geom_area(aes(Date,Production/1000,fill=Region),position="stack",color="black",linewidth=0.5) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_manual("",values = colors_ua10(),guide = "legend")+
  scale_x_datetime(date_breaks="12 months",date_labels =  "%b\n%Y",expand=c(0,0)) +
  weekly_small()+
...
```{r aeso,warning=FALSE,message=FALSE,echo=FALSE,cache=FALSE}
...
renew_plot<-ggplot(gen_mix%>%filter(!Source %in% c("Natural Gas","Coal", "Trade")),aes(date,gen, group=Source, fill = Source)) +
  geom_area(position="stack")+
  weekly_small()+
  scale_x_datetime(expand = c(0,0),date_breaks="3 days",date_labels = "%b\n%d")+
  #scale_y_continuous(limits=c(0,5990),expand = c(0,0))+
...
aeso_fwds<-ggplot(df1 %>% filter(Type=="FLAT",Inst_Date<=Trade_Date+years(3))) +
  ...
  scale_linetype_manual("",values="21",guide=guide_legend(nrow=1,order=2))+
