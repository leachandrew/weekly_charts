...
```{r , child = 'stories/keystone_spill.Rmd'}
```

```{r , child = 'stories/b2ustorage.Rmd'}
```

```{r , child = 'stories/IRA_Europe.Rmd'}
```
...
```{r diffs_5, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H"}
names<-c("Brent","Urals")
diffs_chart(data=data,names,5,curr="USD")
```
