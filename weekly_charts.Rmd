---
title: Leach's Occasional Energy Charts
...
data_fetch<-function(key=KEY, cat=241335){
  ...
  #https://api.eia.gov/v2/petroleum/pri/spt/data/?frequency=daily&data[0]=value&facets[series][]=RWTC&sort[0][column]=period&sort[0][direction]=desc&offset=0&length=5000?&api_key=",key
...
```{r bb_prelims,cache=KEEP_BB_CACHE,warning=FALSE,echo=FALSE,include=FALSE,eval=FALSE}
  #print(getwd())
<<prelims>>
...
```{r eia_spot_prices, cache=KEEP_EIA_CACHE,eval=F} 
...
```{r eia_steo, eval=TRUE, message=FALSE, warning=FALSE, cache=FALSE, results=FALSE,eval=FALSE}
...
<iframe src="https://leach.substack.com/embed" width="480" height="100" style="border:0px solid #EEE; background:white;margin-bottom:-40px" frameborder="0" scrolling="no" data-external="1">
</iframe>
</center>
...
```{r feature_crude, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("Brent","WTI","WCS")
levels_chart(data=data,names,2,curr="USD",title_sent="",break_set="3 months")
```

### Alberta Crude

```{r alberta_crude, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
...
<!-- ```{r alberta_crude_short, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220} -->
<!-- names<-c("Syncrude Sweet Synthetic","Edmonton Mixed Sweet","WCS","Implied Bitumen") -->
...
<!-- ```{r us_crude, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220} -->
...
```{r global_crude, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("Brent","WTI","Tapis","Dubai Fateh","Urals")
levels_chart(data=data,names,5,curr="USD",title_sent="")
```
...
### Brent-WTI
```{r diffs_brent, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("Brent","WTI")
diffs_chart(data=data,names,5,curr="USD")
```

### WTI-WCS
```{r diffs_wcs, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("WTI","WCS")
diffs_chart(data=data,names,5,curr="USD")
```
...
### WTI-WCS (Short)
```{r diffs_wcs_short, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("WTI","WCS")
diffs_chart(data=data,names,1,curr="USD")
```

### Maya-WCS
```{r diffs_maya, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("Maya","WCS")
diffs_chart(data=data,names,5,curr="USD")
```
...
```{r diffs_5, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("Brent","Urals")
diffs_chart(data=data,names,5,curr="USD")
```
...
### WTI
```{r wti_fwd, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_BB_CACHE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
...
### Brent
```{r brent_fwd, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_BB_CACHE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
...
rail_exports<-
  ggplot(oil_by_rail) +
  geom_line(aes(date,volume_bbl_per_day/1000,color="Crude by Rail Exports"),linewidth=1.25) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_x_date(name=NULL,date_breaks="2 years",date_labels =  "%b\n%Y",expand=c(0,0)) +
  scale_color_manual("",values = colors_ua10(),guide = "none")+
  expand_limits(y=0)+
  weekly_small()+
  labs(y="Export Volumes (1000 bbl/d)",x="",
       title=paste("Canadian Oil Exports by Rail",sep=""),
       caption="Source: Canadian Energy Regulator.")

```

```{r cansim_crude, cache=KEEP_CANSIM_CACHE,echo = FALSE, warning=FALSE, message=FALSE,dpi=220}
...
ggplot(aes(group=supply_and_disposition,color=supply_and_disposition))+
  geom_line(aes(Date,value/10^6/days_in_month(month(Date))),linewidth=1.25)+
...
cdn_crude_imports<-crude_disp_new %>% filter(geo=='Canada',supply_and_disposition%in% c("Imports"),
                          uom=="Barrels")%>%
  ggplot(aes(group=supply_and_disposition,color=supply_and_disposition))+
  geom_line(aes(Date,value/10^3/days_in_month(month(Date))),linewidth=1.25)+
  scale_colour_manual(NULL,values=colors_ua10())+
    scale_x_date(date_breaks = "1 year", date_labels =  "%b\n%Y",expand=c(0,0))+
    scale_y_continuous(expand = c(0, 0)) +
    guides(colour="none")+
...
### Canadian Production
```{r cdn_prod_graph, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.height=6.5,fig.pos="H",dpi=220}
cdn_crude_production
```

### Alberta Production
```{r ab_prod_graph, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10,fig.height=6.5, fig.pos="H",dpi=220}
load("st3_prod.gph")
all_crude_weekly
```

### Imports
```{r cdn_imports_graph, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220}
cdn_crude_imports
```

### Exports
```{r cdn_exports_graph, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220}
cdn_crude_exports
```


### Exports by Rail
```{r cbr_graph, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220}
rail_exports
```
...
```{r wpsr_setup, cache=TRUE,eval=F}
...
```{r dpr_data, echo = TRUE,message=FALSE,  include=FALSE, warning=FALSE,cache=KEEP_EIA_CACHE,dpi=220}
...
```{r dpr_oil_play, echo = FALSE, warning=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H",dpi=220}
lto_dpr
```  
...
<!-- ```{r us_crude_prod,eval=F, echo = FALSE, warning=FALSE, message=FALSE,results="asis", cache=FALSE,fig.width=10,fig.pos="H",dpi=220} -->
...
<!-- ```{r us_crude_imp, echo = FALSE, warning=FALSE,message=FALSE,results="asis", cache=FALSE, fig.width=10, fig.pos="H",eval=F,dpi=220} -->
...
<!-- ```{r us_crude_net_imp, eval=F,echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H",dpi=220} -->
...
<!-- ```{r us_crude_inv, eval=F,echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H",dpi=220} -->
...
```{r kalibrate, echo = FALSE, warning=FALSE, message=FALSE,cache=KEEP_KALIBRATE_CACHE,eval=FALSE}
read_chunk('../kalibrate/kalibrate_gas_data.R')
```

```{r gas_data, results=FALSE,echo = FALSE, warning=FALSE, message=FALSE,cache=KEEP_KALIBRATE_CACHE,eval=FALSE}
# run the gas map and charts chunks and use the variables it creates
<<gas_prelims>>
<<gas_load>>
```

```{r gas_maps, echo = FALSE, warning=FALSE, message=FALSE,cache=KEEP_KALIBRATE_CACHE,results=FALSE,eval=FALSE}
# run the gas map and charts chunks and use the variables it creates
<<gas_weekly>>
<<gas_map>>
```
...
```{r map, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_KALIBRATE_CACHE, fig.width=10,fig.height=7, fig.pos="H",dpi=220,eval=FALSE}
load("weekly_map.gph")
weekly_map
```
...
```{r gas_histories, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_KALIBRATE_CACHE, fig.width=10,fig.height=7, fig.pos="H",dpi=220,eval=FALSE}
load("gas_plot.gph")
weekly_gas_plot
```
...
```{r diesel_histories, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_KALIBRATE_CACHE, fig.width=10,fig.height=7, fig.pos="H",dpi=220,eval=FALSE}
...
```{r CER_ref_runs, cache=KEEP_CER_CACHE,eval=FALSE}
...
west_runs<-ggplot(filter(ref_runs,Region=="Western Canada")) +
  ...
  scale_colour_manual("",labels=c("52 Week Average"),values=colors_ua10()[1])+
  #scale_fill_manual("",labels=c("Weekly Throughput"),values=c("Red"))+
  #scale_fill_viridis("",discrete=TRUE,option="D")+
  scale_x_date(name=NULL,date_breaks="2 years",date_labels =  "%b\n%Y",expand=c(0,0)) +
  scale_fill_manual("",values = colors_ua10()[2],guide = "legend",labels="Weekly Runs")+
  guides(colour = guide_legend(order = 1), 
         fill = guide_legend(order = 2))+
  #scale_y_continuous(expand = c(0, 0)) +
  #scale_x_continuous(expand = c(0, 0)) +
...
```{r cdn_ref_runs, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_CER_CACHE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
ref_run_graph 
```


### Western Canada Refinery Runs
```{r west_ref_runs, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_CER_CACHE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
west_runs 
```
...
```{r feature_gas, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
...
```{r feature_gas_short, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names<-c("AECO NIT","Henry Hub","Dawn","Station 2")
...
```{r global_gas_long, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
data<-data %>% mutate(`NBP UK`=`NBP Gas`*`GBP USD`/10)
names<-c("AECO NIT","Henry Hub","Japan LNG JCC","NBP UK")
levels_chart_gas(data=data,names,15*12,"USD")
```

### Global Gas (short)

```{r global_gas, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
data<-data %>% mutate(`NBP UK`=`NBP Gas`*`GBP USD`/10)
names<-c("AECO NIT","Henry Hub","Japan LNG JCC","NBP UK")
levels_chart_gas(data=data,names,24,"USD")
```


### North American Gas Hubs

```{r na_gas, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_BB_CACHE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
names(data)<-gsub("So Cal Gas Co CityGate","SoCal Citygate",names(data))
names(data)<-gsub(" Citygate","",names(data))
names<-c("Dawn","AECO NIT","Chicago","Algonquin","SoCal","PGE")
levels_chart_gas(data=data,names,12,"USD",break_set="2 months",title_sent="Natural Gas Daily Spot Prices")
```



```{r NGX_forwards,cache=KEEP_BB_CACHE,eval=FALSE}
<<NGX_forwards>>
```
...
### Henry Hub
```{r hh_fwd, echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_BB_CACHE, fig.width=10, fig.pos="H",dpi=220,eval=FALSE}
...
### Alberta (NIT) ($CAD)
```{r NGX_nit_cad,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE,dpi=220,eval=FALSE}
...
### Alberta (NIT)
```{r NGX_nit,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE,dpi=220,eval=FALSE}
date_max<-max(ngx_data$trade_date)
series_sent<-c("AB-NIT Fixed")
date_list<-rev(c(date_max,date_max-months(1)))
label_list<-c("Alberta NIT")
...
### BC (Stn 2) 
```{r NGX_stn2,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE,dpi=220,eval=FALSE}
date_max<-max(ngx_data$trade_date)
...
### Ontario (Dawn)
```{r NGX_dawn,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE,dpi=220,eval=FALSE}
date_max<-max(ngx_data$trade_date)
...
## Natural Gas {.tabset .tabset-fade}

### WCSB Production
```{r wcsb_gas,echo = FALSE, message=FALSE,warning=FALSE, results="asis", cache=KEEP_CER_CACHE, fig.width=10, fig.pos="H",dpi=220}

#download.file("https://www.cer-rec.gc.ca/en/data-analysis/energy-commodities/natural-gas/statistics/marketable-natural-gas-production-in-canada-production-de-gaz-commercialisable-#au-canada.xlsx", mode = "wb")

download.file("https://www.cer-rec.gc.ca/en/data-analysis/energy-commodities/natural-gas/statistics/marketable-natural-gas-production-canada-production-gaz-naturel-commercialisable-canada.xlsx",destfile="gas_prod.xlsx",mode="wb")
              
#test<-openxlsx::read.xlsx("gas_prod.xlsx", sheet = "2000+ - 103m3d 103m3j",skipEmptyRows = T)

cer_gas_prod<-readxl::read_excel("gas_prod.xlsx", sheet = "2000+ - 103m3d 103m3j", range = NULL, col_names = TRUE,
                        col_types = NULL, na = "", trim_ws = TRUE, skip = 8) %>% select(-2,-10,-12,-13)%>%
    pivot_longer(-Date,names_to="Region",values_to = "Production")%>%
  mutate(WCSB=Region%in%c("Alberta","BC","Sask"))%>%
  group_by(Region)%>%
  fill(Production, .direction=c("up")) %>%
  ungroup()%>%
  filter(!is.na(Date),!is.na(Production))%>%
  filter(Date<max(Date))
  
  

ggplot(cer_gas_prod%>%filter(WCSB,Date>=ymd("2015-01-01"))) +
  geom_area(aes(Date,Production/1000,fill=Region),position="stack",color="black",linewidth=0.5) +
  scale_y_continuous(expand = c(0, 0)) +
  expand_limits(y=550)+
  scale_fill_manual("",values = colors_ua10(),guide = "legend")+
  scale_x_datetime(date_breaks="6 months",date_labels="%b\n%Y",expand=c(0,0)) +
  weekly_small()+
  labs(y="Marketable Gas Production (Bcf/d)",x="",
       title=paste("WCSB Marketable Gas Production",sep=""),
...
### US Shale Production
```{r dpr_gas_play,echo = FALSE, warning=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H",dpi=220}
gas_dpr
```  

<!-- ### US Storage -->

<!-- ```{r nat_gas_store,eval=F, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10,dpi=220, fig.pos="H"} -->
...
```{r aeso,warning=FALSE,message=FALSE,echo=FALSE,cache=KEEP_AESO_CACHE}
...
price_ail<-get_forecast_report(as.Date(day), as.Date(Sys.Date()))
#day<-Sys.Date()+days(1)
#get last month of data
...
gen_mix <- read.csv(paste(nrg_folder,"AESO_Gen_Mix.csv",sep="/"),skip = 0,header = TRUE, stringsAsFactors=FALSE)
...
#gen_mix_plot
gen_mix_plot_short<-
  ggplot(gen_mix%>%filter(date(date)>=today()-days(7),!is.na(gen)),aes(date,gen, group=Source, fill = Source)) +
  geom_area(position="stack")+
  weekly_small()+
  ...
  #theme(legend.position = "none")+
  labs(x="",y="Supply (MW)",
       title="Alberta Supply Mix, Past 7 Days",
       caption="Source: AESO Data, accessed via NRGStream")
...
load_share_fcast_plot<-
  ggplot(
    renew_fcast%>%left_join(price_ail %>% select(time,forecast_ail),by=c("date"="time"))
    ) +
  geom_ribbon(aes(x = date, ymin =min/forecast_ail , ymax = max/forecast_ail,fill="B"))+
  geom_line(aes(date,most_likely/forecast_ail,colour="A"), size=1.25)+
  weekly_small()+
  scale_x_datetime(expand = c(0,0),labels = date_format("%b %d\n%H:00"),breaks = "1 day")+
  #scale_y_continuous(limits=c(0,5990),expand = c(0,0))+
  scale_color_manual("",labels=c("Most Likely Renewable Generation"),values="#FFDB05")+
...
```{r prices_and_loads,fig.width=10, fig.pos="H",warning=FALSE,message=FALSE,cache=KEEP_AESO_CACHE}
...
```{r aeso_prices_plot, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
...
```{r aeso_mix_plot, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
gen_mix_plot
```

### Supply (7 Days)

```{r aeso_mix_short, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
gen_mix_plot_short
```
...
```{r aeso_fwd_prices, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=F, fig.width=10,dpi=220, fig.pos="H"}
aeso_fwds

```

## AESO Renewables {.tabset .tabset-fade}


### Renewable Supply

```{r aeso_renew_plot, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
renew_plot
```



```{r aeso_gen_mix_long,fig.width=10, fig.pos="H",cache=KEEP_NRGSTREAM_CACHE}
...
gen_mix_long_plot<-ggplot(gen_mix_long,aes(date,gen, group=Source, fill = Source)) +
  geom_area(position="stack")+
  weekly_small()+
  scale_x_datetime(expand = c(0,0),date_breaks="3 days",date_labels = "%b\n%d")+
  #scale_y_continuous(limits=c(0,5990),expand = c(0,0))+
...
```{r aeso_renew_gen_plot, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
grid.arrange(arrangeGrob(top_panel + scale_color_manual("",values=c(colors_ua10()[1],colors_ua10()[1]))+ theme(legend.position="none",
                                           legend.margin=margin(c(0,0,0,0),unit="cm"),
                                           legend.text = element_text(colour="black", size = 14, face = "bold"),
                                           plot.caption = element_text(size = 12, face = "italic"),
                                           plot.title = element_text(size = 14,face = "bold"),
...
```{r aeso_renew_fcast_plot, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
combi_fcast_plot
```
...
```{r wind_plot, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
wind_fcast_plot
```
...
```{r solar_plot, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_AESO_CACHE, fig.width=10, fig.pos="H",dpi=220}
solar_fcast_plot
```
...
<tr>
<td>
[Apr 28, 2022](https://leachandrew.github.io/weekly_charts/weekly_4_28-22.html)
</td>
<td>
...
<tr>
<td>
[Jun 3, 2022](https://leachandrew.github.io/weekly_charts/2022_06_03.html)
</td>
<td>
[Jun 10, 2022](https://leachandrew.github.io/weekly_charts/2022_06_10.html)
</td>
<td>
[Jun 17, 2022](https://leachandrew.github.io/weekly_charts/2022_06_17.html)
</td>
<td>
[Jun 24, 2022](https://leachandrew.github.io/weekly_charts/2022_06_24.html)
</td>
<td>
[Jul 2, 2022](https://leachandrew.github.io/weekly_charts/2022_07_02.html)
</td>
</tr>
<tr>
<td>
[Jul 8, 2022](https://leachandrew.github.io/weekly_charts/2022_07_08.html)
</td>
<td>
[Jul 22, 2022](https://leachandrew.github.io/weekly_charts/2022_07_22.html)
</td>
<td>
[Jul 29, 2022](https://leachandrew.github.io/weekly_charts/2022_07_29.html)
</td>
<td>
[Aug 5, 2022](https://leachandrew.github.io/weekly_charts/2022_08_05.html)
</td>
<td>
[Aug 15, 2022](https://leachandrew.github.io/weekly_charts/2022_08_15.html)
</td>
</tr>
<tr>
<td>
[Aug 19, 2022](https://leachandrew.github.io/weekly_charts/2022_08_19.html)
</td>
<td>
[Sept 9, 2022](https://leachandrew.github.io/weekly_charts/2022_09_09.html)
</td>
<td>
[Sept 15, 2022](https://leachandrew.github.io/weekly_charts/2022_09_15.html)
</td>
<td>
[Sept 25, 2022](https://leachandrew.github.io/weekly_charts/2022_09_25.html)
</td>
<td>
[Sept 30, 2022](https://leachandrew.github.io/weekly_charts/2022_09_30.html)
</td>
</tr>
<tr>
<td>
[Oct 14, 2022](https://leachandrew.github.io/weekly_charts/2022-10-14.html)
</td>
<td>
[Oct 21, 2022](https://leachandrew.github.io/weekly_charts/2022_10_21.html)
</td>
<td>
[Oct 28, 2022](https://leachandrew.github.io/weekly_charts/2022_10_28.html)
</td>
<td>
...
<td>
[Jan 6, 2023](https://leachandrew.github.io/weekly_charts/2023_01_06.html)
</td>
<td>
[Jan 13, 2023](https://leachandrew.github.io/weekly_charts/2023_01_13.html)
</td>
<td>
[Jan 20, 2023](https://leachandrew.github.io/weekly_charts/2023_01_20.html)
</td>
<td>
[Jan 27, 2023](https://leachandrew.github.io/weekly_charts/2023_01_27.html)
</td>
</tr>
<tr>
<td>
[Feb 6, 2023](https://leachandrew.github.io/weekly_charts/2023_02_06.html)
</td>
<td>
[Feb 17, 2023](https://leachandrew.github.io/weekly_charts/2023_02_17.html)
</td>
<td>
[March 3, 2023](https://leachandrew.github.io/weekly_charts/2023_03_03.html)
</td>
<td>
[March 20, 2023](https://leachandrew.github.io/weekly_charts/2023_03_20.html)
</td>
<td>
[April 13, 2023](https://leachandrew.github.io/weekly_charts/2023_04_13.html)
</td>
...
<center>
  Sign up to receive our email updates via [Substack](https://leach.substack.com/)<br>
<iframe src="https://leach.substack.com/embed" width="480" height="100" style="border:0px solid #EEE; background:white;margin-bottom:-40px" frameborder="0" scrolling="no" data-external="1"></iframe>
</center><br><br>
Brought to you by Andrew Leach and the [FES](https://www.ualberta.ca/energy-systems/research.html) Grids and Storage Research Team. <br><br>
