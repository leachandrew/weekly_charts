...
```{r story_3, child = 'stories/climate_vs_security.Rmd'}
```


```{r story_1, child = 'stories/tree_losses.Rmd'}
```


```{r story_2, child = 'stories/renewables_good.Rmd'}
```


```{r child_4, child = 'stories/cali_renew.Rmd'}
```


```{r child_5, child = 'stories/andrew_steo.Rmd'}
```
...
aeso_fwds<-ggplot(subset(df1,Trade_Date %in% dates)) +
  ...
  geom_line(data=subset(df1,Trade_Date %in% dates & Type=="FLAT"),aes(Inst_Date,Settle,colour=as.factor(Trade_Date),group=as.factor(Trade_Date),linetype=Type),size=1.25)+
