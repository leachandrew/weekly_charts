...
data_fetch<-function(key=KEY, cat=241335){
  #key<-KEY
  #cat=476336
  url <-ifelse(cat==999999999,
          paste("https://api.eia.gov/v1/category/?api_key=",
                      key, "&out=xml", sep="" ),
          paste("https://api.eia.gov/v1/category/?api_key=",
...
```{r eia_spot_prices, cache=KEEP_EIA_CACHE,eval=FALSE} 
...
```{r , child = 'stories/TMX.Rmd'}
```

```{r , child = 'stories/G7.Rmd'}
```

```{r , child = 'stories/Russia.Rmd'}
```

```{r , child = 'stories/loopholes.Rmd'}
```

```{r , child = 'stories/ShellPh.Rmd'}
```
...
names<-c("Syncrude Sweet Synthetic","Edmonton Mixed Sweet","WCS","Implied Bitumen")
levels_chart(data=data,names,1,curr="USD",break_set="2 months")
```
...
## US Oil (missing this week due to EIA API changes) {.tabset .tabset-fade}

```{r wpsr_setup, cache=TRUE,eval=FALSE}
...
### Production
```{r us_crude_prod,eval=FALSE, echo = FALSE, warning=FALSE, message=FALSE,results="asis", cache=FALSE, fig.width=10, fig.pos="H"}
...
```{r us_crude_imp,eval=FALSE, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=FALSE, fig.width=10, fig.pos="H"}
...
```{r us_crude_net_imp, eval=FALSE,echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H"}
...
```{r us_crude_inv, eval=FALSE,echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H"}
...
### Historic Gasoline Prices in BTC
...
### Storage (missing this week due to EIA API changes)

```{r nat_gas_store,eval=FALSE, echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H"}
...
<a href="https://www.ualberta.ca/energy-systems/research.html">Brought to you by CABREE, Andrew Leach, Tim Weis, and our FES Grids and Storage Research Team. Thanks to NRGStream for data accesss!</a></p>
