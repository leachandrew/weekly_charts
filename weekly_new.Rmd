...
#TOC {
  ...
  background-size: auto 80px,auto 50px;
  background-repeat: no-repeat,no-repeat;
  background-position: 50% 7px,50% 95%;
}
</style>
...
```{r first,cache=KEEP_BB_CACHE,warning=FALSE,echo=FALSE,include=FALSE}
  #print(getwd())
<<prelims>>
#head(data.frame(x,y))
```
```{r spot_prices, cache=KEEP_EIA_CACHE} 
...
```{r child = 'stories/story_1.Rmd'}
```

```{r child = 'stories/story_2.Rmd'}
```


```{r child = 'stories/story_3.Rmd'}
```

```{r child = 'stories/story_4.Rmd'}
```
...
### WTI
```{r echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_BB_CACHE, fig.width=10, fig.pos="H"}
...
```{r echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=TRUE, fig.width=10,fig.height=7, fig.pos="H"}
weekly_map
```

### Historic Gasoline Prices

```{r echo = FALSE, warning=FALSE, message=FALSE, results="asis", cache=KEEP_BB_CACHE, fig.width=10,fig.height=7, fig.pos="H"}
gas_plot
```
...
### Alberta (NIT) ($CAD)
```{r NGX1a,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE,}
...
```
### Alberta (NIT)
```{r NGX1b,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE}
date_max<-max(ngx_data$trade_date)
series_sent<-c("AB-NIT Fixed")
date_list<-rev(c(date_max,date_max-months(1)))
label_list<-c("Alberta NIT")
...
### BC (Stn 2) 
```{r NGX2,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE}
date_max<-max(ngx_data$trade_date)
...
### Ontario (Dawn)
```{r NGX4,fig.width=10, fig.pos="H",cache=KEEP_BB_CACHE}
date_max<-max(ngx_data$trade_date)
...
```{r echo = FALSE, warning=FALSE,message=FALSE, results="asis", cache=KEEP_EIA_CACHE, fig.width=10, fig.pos="H"}
