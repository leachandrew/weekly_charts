...
```{r EIA_KEY, include=FALSE,cache=FALSE}
...
### Alberta (NIT) ($CAD)
```{r NGX1a,fig.width=10, fig.pos="H"}
...
```
### Alberta (NIT)
```{r NGX1b,fig.width=10, fig.pos="H"}
date_max<-max(ngx_data$trade_date)
series_sent<-c("AB-NIT Fixed")
date_list<-rev(c(date_max,date_max-months(1)))
label_list<-c("Alberta NIT")
...
```{r power functions,warning=FALSE,message=FALSE,echo=FALSE}
...
```{r aeso,cache=TRUE,warning=FALSE,message=FALSE,echo=FALSE}
...
```{r aeso_gen_mix,fig.width=10, fig.pos="H"}
gen_mix <- read.csv("../alberta_power/AESO_Gen_Mix.csv",skip = 2,header = TRUE, stringsAsFactors=FALSE)
names(gen_mix)<-c("date","Coal","Hydro","Trade","Natural Gas","Other","Wind")
gen_mix$date<-dmy_hms(gen_mix$date)
gen_mix<-melt(gen_mix,id="date",variable.name = "Source",value.name = "gen")
#change factor order to sort by avg gen
gen_mix$Source<-factor(gen_mix$Source, levels=c("Hydro","Wind","Other","Natural Gas","Coal","Trade"))
gen_mix_plot<-ggplot(gen_mix,aes(date,gen, group=Source, fill = Source)) +
  ...
  scale_fill_manual("",values=c(colors_ua10()[4],colors_ua10()[1],colors_ua10()[3],colors_ua10()[2],colors_ua10()[5],colors_ua10()[6]))+
  guides(fill = guide_legend(nrow = 1))+
  #theme(legend.position = "none")+
  labs(x="Year",y="Average Hourly Generation (MW)",
       title="Hourly Generation Mix, Past 30 Days",
...
```{r prices_and_loads,,fig.width=10, fig.pos="H",warning=FALSE}
...
new_forwards<-function(){
  ...
  forwards<-proc_forwards("../alberta_power/forwards2018.csv","FLAT")
  peak_forwards<-proc_forwards("../alberta_power/peak_forwards.csv","PEAK")
  ext_peak_forwards<-proc_forwards("../alberta_power/ext_peak_forwards.csv","EXT_PEAK")
...
&nbsp;
<hr />
  <p style="text-align: center;"><a href="https://www.ualberta.ca/energy-systems/research.html">Brought to you by CABREE, Andrew Leach, Tim Weis, and our FES Grids and Storage Research Team</a></p>
